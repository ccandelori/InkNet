                ...
                VStack(alignment: .leading) {
                    ...
                    if let coop = scheduleStore.getCoopGroupingSchedules() {
                        ...
                        ZStack(alignment: .topLeading) {
                            HStack {
                                StageImage(imageURL: current.setting.coopStage.image.url,
                                           stageName: current.setting.coopStage.name)

                                StageParameters(bossName: current.setting.boss.name,
                        ...
                        ZStack(alignment: .topLeading) {
                            ...
                            VStack(alignment: .leading) {
                                ForEach(coop[1...], id: \.startTime) { upcoming in
                                    HStack {
                                        StageImage(imageURL: upcoming.setting.coopStage.thumbnailImage!.url,
                                                   stageName: upcoming.setting.coopStage.name)
                                        .frame(height: 65)

                                        VStack {
                                            ShadedSplatoon2Text(text: "\(scheduleStore.formatDateTime(from: upcoming.startTime)) - \(scheduleStore.formatDateTime(from: upcoming.endTime))",
                                                                size: 12.0)
                                            HStack {
